...
if "history_data" not in st.session_state:
    if os.path.exists(HISTORIC_FILE):
        try:
            st.session_state.history_data = pd.read_csv(HISTORIC_FILE)
        except:
            st.session_state.history_data = pd.DataFrame(columns=["Fecha", "Total_Units", "Avg_Aging_Days", "Critical_Units_120", "Total_Risk_USD"])
    else:
        st.session_state.history_data = pd.DataFrame(columns=["Fecha", "Total_Units", "Avg_Aging_Days", "Critical_Units_120", "Total_Risk_USD"])
...
if uploaded_file:
    ...
    hist = hist[hist["Fecha"] != fecha_reporte]
    st.session_state.history_data = pd.concat([hist, new_row], ignore_index=True).sort_values("Fecha").reset_index(drop=True)
    st.session_state.history_data.to_csv(HISTORIC_FILE, index=False)
    ...
    st.markdown("---")
    st.markdown("<h3 style='color: #FF4B4B;'>🚨 Zona de Riesgo Crítico (> 120 Días Target Cliente)</h3>", unsafe_allow_index=True)
...
h_df = st.session_state.history_data
if not h_df.empty:
    fig_line_units = px.line(h_df, x="Fecha", y=["Total_Units", "Critical_Units_120"], title="Evolución del Backlog (Total vs >120 Días)", markers=True)
    fig_line_cost = px.line(h_df, x="Fecha", y="Total_Risk_USD", title="Tendencia del Valor Financiero en Riesgo ($ USD)", markers=True)
